...
import { createAdminCategory, deleteAdminCategory, getAdminCategories, getAdminJobsModeration, updateAdminCategory } from "../../imports/api";
...
export default function AdminCategories() {
  ...
  useEffect(() => { (async () => {
    try {
      ...
      console.info("Categories endpoint unavailable; deriving categories from jobs.", error);
      try {
        const response = await getAdminJobsModeration(); const data = response?.data ?? response;
        const jobs = Array.isArray(data) ? data : data?.jobs ?? data?.data ?? [];
        const grouped = jobs.reduce((map: Map<string, number>, job: any) => { const category = job.category?.name ?? job.category_name ?? job.department ?? job.dept ?? "Uncategorized"; map.set(category, (map.get(category) ?? 0) + 1); return map; }, new Map<string, number>());
        setItems(Array.from(grouped, ([name, jobs_count], index) => ({ id: index + 1, name, jobs_count })));
        setReadOnly(true);
      } catch (fallbackError) { console.error(fallbackError); toast.error("Could not load job categories"); }
    } finally { setLoading(false); }
  })(); }, []);
  const filtered = useMemo(() => items.filter((item) => item.name.toLowerCase().includes(query.toLowerCase().trim())), [items, query]);
  const openForm = (category?: Category) => { if (readOnly) { toast.info("Category management needs the Laravel categories endpoints."); return; } setForm(category ?? "new"); setName(category?.name ?? ""); };
  const save = async () => {
  ...
  return <div className="admin-categories-page" style={{ fontFamily: F, color: C.text }}>
    {readOnly && <div style={{ padding: "11px 14px", marginBottom: 16, borderRadius: 12, background: C.warningBg, color: C.warning, fontSize: 12 }}>Categories are derived from existing jobs. Add the Laravel <b>/api/admin/categories</b> endpoints to enable create, edit and delete.</div>}
